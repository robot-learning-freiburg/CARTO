...
def get_2d_bbox_of_9D_box(camera_T_object, scale_matrix, camera_model):
    ...
    bbox = [
        np.array([np.min(morphed_pixels[:, 0]), np.min(morphed_pixels[:, 1])]),
        np.array([np.max(morphed_pixels[:, 0]), np.max(morphed_pixels[:, 1])]),
    ]
    return bbox
...
def project_pose_onto_image(pose, camera_model):
    ...
    morphed_pixels = camera.convert_homopixels_to_pixels(
        camera_model.project(morphed_homopoints)
    ).T
...
def test_pose_solver():
    ...
    world_T_camera = np.array(
        [
            [0.99376416, -0.05495078, 0.0970217, 1.43237753],
            [0.00736171, 0.90056662, 0.43465568, 0.72350959],
            [-0.11125918, -0.43123099, 0.89535536, 3.94788388],
            [0.0, 0.0, 0.0, 1.0],
        ]
    )
    target_t_points = np.linalg.inv(world_T_camera) @ target_t_points
    pixels_target = camera.convert_homopixels_to_pixels(
        camera.Camera().project(target_t_points)
    )
    pixels_target_gt = np.array(
        [
            [198.60924037, 181.08880028],
            [-384.55689665, 452.51438988],
            [197.90794419, 176.84519486],
            [-405.05431808, 439.75925094],
            [451.6781963, 190.51153042],
            [1088.99607868, 718.53052576],
            [452.57505678, 185.91876331],
            [1128.80169693, 709.80385466],
        ]
    )
    # target_t_points = camera.Camera().RT_matrix @ target_t_points
    # pixels_target = points_to_camera(target_t_points)
    # print("Custom projection ",np.round(pixels_target,3))
    _, camera_T_object, scale_matrix = optimize_for_9D(
        pixels_target, solve_for_transforms=True
    )
    ...
    camera_homopixels = camera.Camera().K_matrix @ target_t_points
