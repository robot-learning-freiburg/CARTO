...
class NetFactory(nn.Module):
    def __init__(self, model_norm="BN"):
        super().__init__()
        self.nodes = []
        self.skips = {}
        self.tags = {}
    ...
    def _activate(self, node):
        if node.activated:
            return node
        if self.model_norm == "BN":
            norm = nn.BatchNorm2d(node.dim)
        elif self.model_norm == "GN":
            if node.dim == 32:
                N = 16
            else:
                N = 32
            norm = nn.GroupNorm(N, node.dim)
        else:
        ...
        return self._add(
            dataclasses.replace(
                node,
                inp=node,
                module=nn.Sequential(norm, nn.LeakyReLU(inplace=True)),
                activated=True,
            )
        )
    ...
    def bake(self):
        self.modules = nn.ModuleList(
            n.module for n in self.nodes if n.module is not None
        )
        return self
...
class HdrnAlphaStereo(nn.Module):
    def __init__(self, hparams):
        ...
        self.process_cost_volume = make_process_cost_volume(
            self.internal_num_disparities
        )
...
class StereoBackbone(nn.Module):
    def __init__(self, hparams, in_channels=3, in_dim=96):
        super().__init__()
        ...
        def make_rgbd_backbone(in_dim=96, num_channels=64, out_dim=64):
            net = NetFactory()
            x = net.input(in_dim=in_dim, activated=True, stride=4)
        ...
        self.rgbd_backbone = make_rgbd_backbone(in_dim=in_dim)

    def forward(self, stacked_img, language_features=None):
        small_disp = self.stereo_stem(stacked_img[:, 0:3], stacked_img[:, 3:6])
        left_rgb_features = self.rgb_stem(stacked_img[:, 0:3])
        disp_features = self.disp_features(small_disp)

        if language_features != None:
            rgbd_features = torch.cat(
                (disp_features, left_rgb_features, language_features), axis=1
            )
        else:
            rgbd_features = torch.cat((disp_features, left_rgb_features), axis=1)

        outputs = self.rgbd_backbone(rgbd_features)
...
class PrecomputedStereoBackbone(nn.Module):
    def __init__(self, hparams, in_channels=3):
        super().__init__()

        def make_rgbd_stem(hparams):
            net = NetFactory(model_norm=hparams.model_norm)
            x = net.input(in_dim=4, stride=1, activated=True)
            x = net.downscale(x, 32)
            x = net.downscale(x, 32)
            return net.bake()

        self.rgbd_stem = make_rgbd_stem(hparams)

        def make_rgbd_backbone(hparams, num_channels=64, out_dim=64):
            net = NetFactory(model_norm=hparams.model_norm)
            x = net.input(in_dim=32, activated=True, stride=4)
            x = net._lateral(x, out_dim=num_channels)
            x4 = x = net.block(x, "111")
            x = net.downscale(x, num_channels * 2)
            x8 = x = net.block(x, "1111")
        ...
        self.rgbd_backbone = make_rgbd_backbone(hparams)

    def forward(self, stacked_img, robot_joint_angles=None):
        rgbd_features = self.rgbd_stem(stacked_img)
        outputs = self.rgbd_backbone(rgbd_features)
        return outputs
...
def cost_volume(left, right, num_disparities: int, is_right: bool):
    ...
    output = torch.zeros(
        (batch_size, channels, num_disparities, height, width),
        dtype=left.dtype,
        device=left.device,
    )
...
def dot_product_cost_volume(left, right, num_disparities: int, is_right: bool):
    ...
    output = torch.zeros(
        (batch_size, num_disparities, height, width),
        dtype=left.dtype,
        device=left.device,
    )
...
class DotProductCostVolume(nn.Module):
    ...
    def forward_with_amp(self, left, right):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            left = left.to(torch.float32)
            right = right.to(torch.float32)
            output = dot_product_cost_volume(
...
def main():
    ...
    onnx.save(
        shape_inference.infer_shapes(onnx.load("test_net.onnx")), "test_net_shapes.onnx"
    )
