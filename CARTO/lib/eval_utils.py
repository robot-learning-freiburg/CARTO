...
def get_gt_pc(object_id, global_scale, joint_config, n_points):
    object_path = partnet_mobility.PartNetMobilityV0DB.get_object(object_id)
    object_meta = partnet_mobility.PartNetMobilityV0DB.get_object_meta(object_id)

    object_meta = partnet_mobility.PartNetMobilityV0DB.get_object_meta(object_id)
    class_name = object_meta["model_cat"]
    gt_cl_id = categories_to_id[class_name]
    urdf_object = urdfpy.URDF.load(str(object_path / "mobility.urdf"))
    obj_trimesh: trimesh.Scene
    obj_trimesh, _, scale = arti_utils.object_to_trimesh(
        urdf_object=urdf_object,
        joint_config=joint_config,
        base_transform=object_meta["canonical_transformation"],
        # TODO Be careful, this needs to align with the generation config but its nested very deep!
        origin_frame=config.ObjectOrigin.CLOSED_STATE,
        additional_scale=global_scale,
    )

    if type(obj_trimesh) is trimesh.Scene:
        obj_trimesh = obj_trimesh.dump(concatenate=True)
    pc = obj_trimesh.sample(count=n_points)
    return pc, gt_cl_id
...
def get_ids_from_seg(seg_output, output_indices, is_target):
    if seg_output.is_numpy:
        category_seg_output = seg_output.seg_pred
    else:
        category_seg_output = np.ascontiguousarray(seg_output.seg_pred.cpu().numpy())
    if is_target:
        category_seg_output = category_seg_output
    else:
        category_seg_output = np.argmax(category_seg_output, axis=1)[0]
    class_ids_predicted = []
    for k in range(len(output_indices)):
        center = output_indices[k]
        class_ids_predicted.append(category_seg_output[center[0], center[1]])
...
def compute_3d_IoU(sRT_1, sRT_2, size_1, size_2):
    """Computes IoU overlaps between two 3D bboxes."""

    assert size_1.shape[0] == 6 or size_1.shape[0] == 3
    assert size_2.shape[0] == 6 or size_2.shape[0] == 3

    def asymmetric_3d_iou(sRT_1, sRT_2, size_1, size_2):
        if size_1.shape[0] == 3:
            noc_cube_1 = get_3d_bbox(size_1, 0)
        elif size_1.shape[0] == 6:
            noc_cube_1 = get_3d_asymetric_bbox(size_1[:3], size_1[3:]).T
        else:
            assert False, "Invalid size_1.shape"
        bbox_3d_1 = transform_coordinates_3d(noc_cube_1, sRT_1)

        if size_2.shape[0] == 3:
            noc_cube_2 = get_3d_bbox(size_2, 0)
        elif size_1.shape[0] == 6:
            noc_cube_2 = get_3d_asymetric_bbox(size_2[:3], size_2[3:]).T
        else:
            assert False, "Invalid size_2.shape"
        bbox_3d_2 = transform_coordinates_3d(noc_cube_2, sRT_2)

        # new from ssc-6d (decreased results iou slightly, we are going to stick with original implementation/one used in CS paper)
...
def compute_simple_3d_IoU(sRT_1, size_1, bbox_3d_2, class_name_1, class_name_2):
    """Computes simple IoU overlaps between two 3D bboxes."""

    def asymmetric_3d_iou(bbox_3d_2, sRT_1, size_1):
        noc_cube_1 = get_3d_bbox(size_1, 0)
        bbox_3d_1 = transform_coordinates_3d(noc_cube_1, sRT_1)
    ...
    if class_name_1 in ["bottle", "bowl", "can"] and class_name_1 == class_name_2:
        ...
        for i in range(n):
            ...
            max_iou = max(max_iou, asymmetric_3d_iou(bbox_3d_2, rotated_RT_1, size_1))
    else:
        max_iou = asymmetric_3d_iou(bbox_3d_2, sRT_1, size_1)
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j] = compute_3d_IoU(
                pred_sRT[i], gt_sRT[j], pred_size[i, :], gt_size[j]
            )
...
def compute_RT_overlaps(gt_class_ids, gt_sRT, pred_class_ids, pred_sRT):
    ...
    for i in range(num_pred):
        for j in range(num_gt):
            overlaps[i, j, :] = compute_RT_errors(pred_sRT[i], gt_sRT[j])
...
def get_categories_for_mean(return_all: bool = False):
    all_categories = [
        "BG",
        "Dishwasher",
        "Knife",
    ...
    if not return_all:
        return categories_for_mean
    return categories_for_mean, all_categories
...
def compute_mAP(
    ...
    for progress, result in enumerate(tqdm(pred_results)):
        ...
        # This is hard-coded here to not break when testing in a zero-shot way i.e. when we have empty or zero predictions from the net
        ...
        for cls_id in range(1, num_classes):
            ...
            # cls_gt_size = gt_size[gt_class_ids == cls_id] if len(gt_class_ids) else np.zeros((0, 3)) # <--- Does this size
            cls_gt_size = (
                gt_size[gt_class_ids == cls_id]
                if len(gt_class_ids)
                else np.zeros((0, 6))
            )  # And this size really matter?
            cls_pred_class_ids = (
            ...
            RT_overlaps = compute_RT_overlaps(
                cls_gt_class_ids, cls_gt_sRT, cls_pred_class_ids, cls_pred_sRT
            )
    ...
    categories_for_mean = np.array(get_categories_for_mean())
    iou_aps[-1, :] = np.mean(iou_aps[categories_for_mean, :], axis=0)
    iou_acc[-1, :] = np.mean(iou_acc[categories_for_mean, :], axis=0)
    ...
    for i, degree_thres in enumerate(degree_thres_list):
        for j, shift_thres in enumerate(shift_thres_list):
            ...
            pose_aps[-1, i, j] = np.mean(pose_aps[categories_for_mean, i, j])
            pose_acc[-1, i, j] = np.mean(pose_acc[categories_for_mean, i, j])
    # save results to pkl
    result_dict = {}
    ...
    return iou_aps, pose_aps, iou_acc, pose_acc, eval_count, total_count
...
def plot_mAP(
    ...
    mean_categories = get_categories_for_mean() + [len(labels)]
    ...
    styles = ["-"] * (len(mean_categories) - 1) + ["--"]
    # styles = ['-', '-', '-', '-', '-', '-', '--', ':']

    # print(pose_aps.shape)
    # print(f"{degree_thres_list = }")
    # print(f"{shift_thres_list = }")

    fig, (ax_iou, ax_degree, ax_shift) = plt.subplots(1, 3, figsize=(8.5, 3.5))  # 3.5))
    # IoU subplot
    ...
    # ax_iou.grid()
    for i in range(0, len(mean_categories)):
        mean_category_id = mean_categories[i]
        ax_iou.plot(
            100 * np.array(iou_thres_list),
            100 * iou_aps[mean_category_id, :],
            color=colors[i],
            linestyle=styles[i],
            label=labels[mean_category_id - 1],
        )
    ...
    ax_degree.set_xlim(0, degree_thres_list[-1])
    ax_degree.xaxis.set_ticks(
        np.linspace(degree_thres_list[0], degree_thres_list[-1], 4 + 1, dtype=int)
    )
    # ax_degree.grid()
    # New
    for i in range(0, len(mean_categories)):
        ...
        ax_degree.plot(
            ...
            100 * pose_aps[mean_category_id, : len(degree_thres_list), -1],
            color=colors[i],
            linestyle=styles[i],
            label=labels[mean_category_id - 1],
        )
    ...
    ax_shift.set_xlim(0, shift_thres_list[-1])
    ax_shift.xaxis.set_ticks(
        np.linspace(shift_thres_list[0], shift_thres_list[-1], 3 + 1, dtype=int)
    )
    # ax_shift.grid()
    for i in range(0, len(mean_categories)):
        mean_category_id = mean_categories[i]
        ax_shift.plot(
            ...
            100 * pose_aps[mean_category_id, -1, : len(shift_thres_list)],
            color=colors[i],
            linestyle=styles[i],
            label=labels[mean_category_id - 1],
        )
    ...
    plt.savefig(os.path.join(out_dir, "mAP.pdf"))
    plt.close(fig)
    return
...
def compute_and_plot_mAP(
    ...
    nocs_results,
    degree_steps=[10, 20],
    shift_steps=[10, 20, 30],
):
    assert len(degree_steps) == 2
    assert len(shift_steps) == 3

    degree_thres_list = list(range(0, min(60, max(degree_steps)) + 1, 1))
    shift_thres_list = [k / 2 for k in range(max(shift_steps) * 2 + 1)]
    iou_thres_list = [k / 100 for k in range(101)]
    result_dir = str(output_path)
    print("output_path", output_path)
    pathlib.Path(output_path).mkdir(parents=True, exist_ok=True)
    iou_aps, pose_aps, iou_acc, pose_acc, eval_count, total_count = compute_mAP(
        pred_results,
        result_dir,
        degree_thres_list,
        shift_thres_list,
    ...
    degree_05_idx = degree_thres_list.index(degree_steps[0])
    degree_10_idx = degree_thres_list.index(degree_steps[1])
    shift_02_idx = shift_thres_list.index(shift_steps[0])
    shift_05_idx = shift_thres_list.index(shift_steps[1])
    shift_10_idx = shift_thres_list.index(shift_steps[2])
    messages = []
    messages.append(f"{eval_count} evaluated out of {total_count}")
    messages.append("mAP:")
    ...
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
            degree_steps[0],
            shift_steps[0],
            pose_aps[-1, degree_05_idx, shift_02_idx] * 100,
        )
    )
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
    ...
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
            degree_steps[1],
            shift_steps[0],
            pose_aps[-1, degree_10_idx, shift_02_idx] * 100,
        )
    )
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
    ...
    for k in range(1, iou_aps.shape[0]):
        messages.append(labels[k - 1])
        messages.append("3D IoU at 25: {:.1f}".format(iou_aps[k, iou_25_idx] * 100))
        messages.append("3D IoU at 50: {:.1f}".format(iou_aps[k, iou_50_idx] * 100))
        messages.append("3D IoU at 75: {:.1f}".format(iou_aps[k, iou_75_idx] * 100))
        messages.append(
            "{:d} degree, {:d}cm: {:.1f}".format(
                degree_steps[0],
                shift_steps[0],
                pose_aps[k, degree_05_idx, shift_02_idx] * 100,
            )
        )
        messages.append(
            "{:d} degree, {:d}cm: {:.1f}".format(
                degree_steps[0],
                shift_steps[1],
                pose_aps[k, degree_05_idx, shift_05_idx] * 100,
            )
        )
        messages.append(
            "{:d} degree, {:d}cm: {:.1f}".format(
                degree_steps[0],
                shift_steps[2],
                pose_aps[k, degree_05_idx, shift_10_idx] * 100,
            )
        )
        messages.append(
            "{:d} degree, {:d}cm: {:.1f}".format(
                degree_steps[1],
                shift_steps[0],
                pose_aps[k, degree_10_idx, shift_02_idx] * 100,
            )
        )
        messages.append(
            "{:d} degree, {:d}cm: {:.1f}".format(
                degree_steps[1],
                shift_steps[1],
                pose_aps[k, degree_10_idx, shift_05_idx] * 100,
            )
        )
        messages.append(
            "{:d} degree, {:d}cm: {:.1f}".format(
                degree_steps[1],
                shift_steps[2],
                pose_aps[k, degree_10_idx, shift_10_idx] * 100,
            )
        )
        messages.append(
            "=========================================================================\n"
    ...
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
            degree_steps[0],
            shift_steps[0],
            pose_acc[-1, degree_05_idx, shift_02_idx] * 100,
        )
    )
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
    ...
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
            degree_steps[1],
            shift_steps[0],
            pose_acc[-1, degree_10_idx, shift_02_idx] * 100,
        )
    )
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
            degree_steps[1],
            shift_steps[1],
            pose_acc[-1, degree_10_idx, shift_05_idx] * 100,
        )
    )
    messages.append(
        "{:d} degree, {:d}cm: {:.1f}".format(
